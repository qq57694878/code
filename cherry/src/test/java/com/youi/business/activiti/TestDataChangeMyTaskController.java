 ...
 */
public class TestDataChangeMyTaskController extends TestRestBussinessBase {
    @Test
    public void getHasdoTaskList()  throws Exception {
        ...
        Response r = RestAssured.given().contentType("application/json").body( objectMapper.writeValueAsString(param))
                ...
                .post(""+PRE_PATH+"/mywork/task/history.do");
    ...
    public void testStartTask()  throws Exception {
        ...
        param.put("title","shujuxiugai"+ i);

        param.put("description","shujuxiugaimiaoshu1"+ i);
        param.put("stime","2016-08-24 10:10:10");
        param.put("etime","2016-08-26 10:10:10");
        param.put("business_app","1");
        param.put("task_type","sw_data_change");
        Response r = RestAssured.given().contentType("multipart/form-data").formParams(param)
               .multiPart("file1",new File("D:\\a\\aa\\1.txt"))
                ...
                .post(""+PRE_PATH+"/task/sw_data_change/start.do");
        r.then().statusCode(200).body("error_code", Matchers.equalTo(200));
        r.prettyPrint();
    }
    ...
    public void testgetTaskDetail()  throws Exception {
        ...
        if(tasks!=null){
            for(Map task:tasks){
                ...
                Response r = RestAssured.given().contentType("application/json").body( objectMapper.writeValueAsString(param))
                        .header(Constants.AUTHENTICATION_HEADER,this.token).header(Constants.VERSION,Constants.DEFAULT_VERSION)
                        .post(""+PRE_PATH+"/task/sw_data_change/get.do");
                r.then().statusCode(200).body("error_code", Matchers.equalTo(200));
                r.prettyPrint();
            }
        }
          ...
          String node_id=  (String)task_info.get("node_id");
                ...
                for(int i=1;i<=6;i++){
                    ...
                    if("step1".equals(node_id)){
                        int i = new Random().nextInt(100);
                        form_data.put("title","shujuxiugaisubmit"+ i);
                        form_data.put("change_type", "2");
                        form_data.put("description","shujuxiugaimiaoshu1submit"+ i);
            ...
            if("step4".equals(node_id)){
                ...
                form_data.put("backup_sql", "insert into sw_data_query_bak as select * from sw_data_query;");
                form_data.put("scheme","处理方案step4");
            }

                param.put("form_data",form_data);
                Response r = RestAssured.given().contentType("application/json").body( objectMapper.writeValueAsString(param))
                        .header(Constants.AUTHENTICATION_HEADER,this.token).header(Constants.VERSION,Constants.DEFAULT_VERSION)
                        .post(""+PRE_PATH+"/task/sw_data_change/submit.do");
                r.then().statusCode(200).body("error_code", Matchers.equalTo(200));
                r.prettyPrint();
            }
    }
    ...
    public void testRejectTask()  throws Exception {
        ...
        if(tasks!=null&&tasks.size()>0){
            ...
            Response r = RestAssured.given().contentType("application/json").body( objectMapper.writeValueAsString(param))
                    .header(Constants.AUTHENTICATION_HEADER,this.token).header(Constants.VERSION,Constants.DEFAULT_VERSION)
                    .post(""+PRE_PATH+"/task/sw_data_change/return.do");
            r.then().statusCode(200).body("error_code", Matchers.equalTo(200));
            r.prettyPrint();
        }
    }
    ...
    public void testStopTask()  throws Exception {
        ...
        if(tasks!=null&&tasks.size()>0){
            ...
            Response r = RestAssured.given().contentType("application/json").body( objectMapper.writeValueAsString(param))
                    .header(Constants.AUTHENTICATION_HEADER,this.token).header(Constants.VERSION,Constants.DEFAULT_VERSION)
                    .post(""+PRE_PATH+"/task/sw_data_change/stop.do");
            r.then().statusCode(200).body("error_code", Matchers.equalTo(200));
            r.prettyPrint();
        }
    }
