...
package com.youi.business.common.interceptor;
 ...
 * {@link }, just for use with AOP setup instead of
...
public class OpenSessionInterceptor implements MethodInterceptor, InitializingBean {
	...
	private Logger logger = LoggerFactory.getLogger(OpenSessionInterceptor.class);
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		if (!TransactionSynchronizationManager.hasResource(sf)) {
			logger.info("session no transaction:无");
			// New Session to be bound for the current method's scope...
			Session session = openSession();
			try {
				TransactionSynchronizationManager.bindResource(sf, new SessionHolder(session));
				logger.info("开启session");
			...
			logger.info("session has transaction:有");
