package com.youi.business.common.snmp;
...
public class MySnmpRequest  implements CommandResponder, PDUFactory {
   ...
   public static class DbOneColumnTableListener implements TableListener {
        private ResSnmpReqResultDao resSnmpReqResultDao;
        private String req_address;
        private String req_oid;
        private long req_time;
        public DbOneColumnTableListener(ResSnmpReqResultDao resSnmpReqResultDao,String req_address,String req_oid,long req_time){
            this.resSnmpReqResultDao = resSnmpReqResultDao;
        ...
        public boolean next(TableEvent event) {
           /* for (int i=0; i<event.getColumns().length; i++) {
            ...
            VariableBinding v = event.getColumns()[0];
            String resp_oid = v.getOid().toString();
            String resp_value = v.getVariable().toString();
            String row_index = event.getIndex().toString();
            RES_SNMP_REQ_RESULT o = new RES_SNMP_REQ_RESULT();
          //  o.setId(resSnmpReqResultDao.getIdGenerator().generateId());
            o.setCtime(new Date(req_time));
            o.setReq_address(this.req_address);
            o.setReq_oid(req_oid);
            o.setReq_time(new Date());
            o.setResp_oid(resp_oid);
            o.setResp_value(resp_value);
            if(Constants.SNMP_REQ_atPhysAddress.equals(this.req_oid)){
