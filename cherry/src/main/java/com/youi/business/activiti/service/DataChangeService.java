...
public class DataChangeService implements UserTaskService {
    private static Logger logger = LoggerFactory
            .getLogger(DataChangeService.class);
    @Autowired
    ...
    @Autowired
    private SwDataChangeDao dataChangeDao;
    @Autowired
    ...
    public Map<String, Object> getTaskFormData(String task_id, String task_type, String bussinessid, String bussiness_type) throws Exception {
        ...
        if ("todo".equals(bussiness_type)) {
            ...
            SW_DATA_CHANGE data_change = dataChangeDao.get(Long.parseLong(bussinessid));
            if (data_change != null) {
                result.put("title", data_change.getTitle());
                result.put("stime", data_change.getStime());
                result.put("etime", data_change.getEtime());
            }
            ...
            if (data_change != null) {
                result.put("title", data_change.getTitle());
                result.put("etime", data_change.getEtime());
                result.put("description", data_change.getDescription());
                result.put("stime", data_change.getStime());
                Long business_app = data_change.getBusiness_app();
                ...
                result.put("id", data_change.getId());


                //附件
                String yw_type = task_type;
                String yw_id = String.valueOf(data_change.getId());
                ...
                String company_name="";
                String leader_name="";
                if(data_change.getCompany_id()!=null){
                    company_name=  CodeTableUtil.code2mean("SYSORG",data_change.getCompany_id());
                    leader_name=  CodeTableUtil.code2mean("SYSUSER",data_change.getLeader_id());
                }
                cocompany.put("company_id", StringUtils.null2String(String.valueOf(data_change.getCompany_id())));
                cocompany.put("company_name", company_name);
                cocompany.put("leader_id", StringUtils.null2String(String.valueOf(data_change.getLeader_id())));
                cocompany.put("leader_name", leader_name);
                result.put("cocompany", cocompany);
                result.put("actually_stime", data_change.getActually_stime()==null?"":data_change.getActually_stime());
                result.put("actually_etime", data_change.getActually_etime()==null?"":data_change.getActually_etime());
                result.put("actually_sql", data_change.getActually_sql()==null?"":data_change.getActually_sql());
                result.put("backup_sql", data_change.getBackup_sql()==null?"":data_change.getBackup_sql());
                result.put("scheme",data_change.getScheme()==null?"":data_change.getScheme());
            }
        }
        return result;
    }
    ...
    public boolean completeTaskFormData(String task_id, Map<String, Object> paramMap, MultiValueMap<String, MultipartFile> multipartFiles) throws Exception {
        ...
        if ("step1".equals(node_id)) {
            //提交需求任务申请
            SW_DATA_CHANGE obj = null;
            if (!StringUtils.isEmpty(bussinesskey)) {
                obj = dataChangeDao.load(Long.parseLong(bussinesskey));
            } else {
                obj = new SW_DATA_CHANGE();
            }
            ...
            //更新外协公司数据
            if (!StringUtils.isEmpty(bussinesskey)) {
                SW_DATA_CHANGE obj = dataChangeDao.load(Long.parseLong(bussinesskey));
                if (formData != null) {
...
*/
            ...
            if (!StringUtils.isEmpty(bussinesskey)) {
                ...
                if (formData != null) {
                    ...
                    if (formData.get("backup_sql") != null) {
                        obj.setBackup_sql((String) formData.get("backup_sql"));
                    }
                    obj.setScheme((String)formData.get("scheme"));
                    dataChangeDao.update(obj);
                }
            }
    ...
    public boolean startTask(Map<String, Object> paramMap, MultiValueMap<String, MultipartFile> multipartFiles) throws Exception {
        ...
        //提交需求任务申请
        SW_DATA_CHANGE obj = new SW_DATA_CHANGE();
