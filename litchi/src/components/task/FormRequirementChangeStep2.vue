		...
		submit:function(){
			...
			bootbox.prompt("请输入意见", function(result) {
				if(result!=null) {
			 		...
			 		// formData.append('task_id',self.taskId);
			 		// formData.append('task_type',self.taskType);
			 		// formData.append('instance_id',self.instance_id);
			 		// formData.append('node_id',self.nodeId);
			 		let requestBody={ task_id:self.taskId,node_id:self.nodeId,task_type:self.taskType,instance_id:self.instanceId,form_data:{opinion:result}};
