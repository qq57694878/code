			...
			<tbody>
				...
				<td>
					...
					<div class="hidden-sm hidden-xs btn-group" v-else>
						<button class="btn btn-success" v-on:click="read(notice.id)"> <i class="ace-icon fa fa-pencil-square-o"></i> 确认 </button>
					</div>
					...
					<div class="hidden-md hidden-lg" v-else>
						<div class="inline position-relative">
							...
							<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
								...
								<li>
								<a href="#" class="tooltip-success" data-rel="tooltip" title="" data-original-title="确认" v-on:click="read(notice.id)">
								<span class="green">
								<i class="ace-icon fa fa-pencil-square-o bigger-120"></i>
								</span>
								</a>
...
import {getReceiveNoticeList} from '../../vuex/modules/notice/NoticeGetters'
import {reqReceiveNoticeList,readNotice} from '../../vuex/modules/notice/NoticeActions'
export default {
	data:function(){
		return {
			pageSize:10,
	...
	vuex:{
		getters:{
			noticesList:getReceiveNoticeList
			//CT:getCodeTable
		},
		actions:{
			reqReceiveNoticeList,
			readNotice
		}
	},
	methods:{
		next:function(pageNumber) {
			...
			this.reqReceiveNoticeList(pageNumber,this.pageSize);
		},
	...
	filters:{
		picFilter:function(value){
			if(value.notice_type=="1")
				...
				return 'fa-ellipsis-h purple';
		}
	},
	ready:function(){
		this.reqReceiveNoticeList(1,10);
	}
}
</script>
