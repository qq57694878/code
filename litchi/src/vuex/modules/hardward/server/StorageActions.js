...
export const reqQueryInit = (store,pageNo,pageSize) => {
  ...
  console.info('reqQueryInit');
  ajaxReq(store,{
    url:'hardware/storage/init.do',
    body:requestBody,
    success:function(response){
      let responseJson=response.json();
      store.dispatch(types.SET_HARDWARD_STORAGE,responseJson.data)
      bootbox.hideAll();
    },
    failure:function(){
      bootbox.hideAll();
...
export const delInfo = (store, input_id) => {
  ...
  ajaxReq(store, {
    url: 'hardware/storage/delete.do',
    body: requestBody,
    success: function (response) {
      let responseJson = response.json();
...
export const reqFormInit = (store, input_id) => {
  ...
  if (input_id == null) {
    let requestBody = {device_type: 'hw_storage'};
    ajaxReq(store, {
      url: 'hardware/common/get_new_asset_num.do',
      body: requestBody,
      success: function (response) {
        let responseJson = response.json();
        store.dispatch(types.SET_HARDWARD_STORAGE_FORM, responseJson.data)
    ...
    let requestBody = {id: input_id};
    ajaxReq(store, {
      url: 'hardware/storage/detail.do',
      body: requestBody,
      success: function (response) {
        let responseJson = response.json();
        store.dispatch(types.SET_HARDWARD_STORAGE_FORM, responseJson.data)
